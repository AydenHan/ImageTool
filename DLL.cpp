...
void Padding(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int padx, int pady, int borderType = 0) {
	int row = srcHeight + pady * 2;
    int col = srcWidth + padx * 2;
    unsigned char* newData = (unsigned char*)malloc(sizeof(unsigned char) * row * col * srcChannel);
	// 先填充src
    for (int i = pady; i < row - pady; ++i) {
        for (int j = padx; j < col - padx; ++j) {
        	for(int c = 0; c < srcChannel; c++) {
        		newData[i * col * srcChannel + j * srcChannel + c] = srcImage[(i - pady) * srcWidth * srcChannel + (j - padx) * srcChannel + c];
			}
        }
    }
	if (borderType == 0) {	// 镜像填充
		// 填充left & right
		int leftBorder = padx;
		for (int i = pady; i < row - pady; ++i) {
			for (int j = 0; j < leftBorder; ++j) {
				for(int c = 0; c < srcChannel; c++) {
					newData[i * col * srcChannel + j * srcChannel + c] = newData[i * col * srcChannel + (leftBorder * 2 - j) * srcChannel + c];
				}
			}
		}
		int rightBorder = col - 1 - padx;
		for (int i = pady; i < row - pady; ++i) {
			for (int j = col - 1; j > rightBorder; --j) {
				for(int c = 0; c < srcChannel; c++) {
					newData[i * col * srcChannel + j * srcChannel + c] = newData[i * col * srcChannel + (rightBorder * 2 - j) * srcChannel + c];
				}
			}
		}
		// 填充top & bottom
		...
		for (int i = 0; i < topBorder; ++i) {
			for (int j = 0; j < col; ++j) {
				for(int c = 0; c < srcChannel; c++) {
					newData[i * col * srcChannel + j * srcChannel + c] = newData[(topBorder * 2 - i) * col * srcChannel + j * srcChannel + c];
				}
			}
		}
		int bottomBorder = row - 1 - pady;
		for (int i = row - 1; i > bottomBorder; --i) {
			for (int j = 0; j < col; ++j) {
				for(int c = 0; c < srcChannel; c++) {
					newData[i * col * srcChannel + j * srcChannel + c] = newData[(bottomBorder * 2 - i) * col * srcChannel + j * srcChannel + c];
				}
			}
		}
	}
	else if (borderType == 1) {		// 复制填充
		// 填充left & right
		int leftBorder = padx;
		for (int i = pady; i < row - pady; ++i) {
			for (int j = 0; j < leftBorder; ++j) {
				for(int c = 0; c < srcChannel; c++) {
					newData[i * col * srcChannel + j * srcChannel + c] = newData[i * col * srcChannel + leftBorder * srcChannel + c];
				}
			}
		}
		int rightBorder = col - 1 - padx;
		for (int i = pady; i < row - pady; ++i) {
			for (int j = col - 1; j > rightBorder; --j) {
				for(int c = 0; c < srcChannel; c++) {
					newData[i * col * srcChannel + j * srcChannel + c] = newData[i * col * srcChannel + rightBorder * srcChannel + c];
				}
			}
		}
		// 填充top & bottom
		...
		for (int i = 0; i < topBorder; ++i) {
			for (int j = 0; j < col; ++j) {
				for(int c = 0; c < srcChannel; c++) {
					newData[i * col * srcChannel + j * srcChannel + c] = newData[topBorder * col * srcChannel + j * srcChannel + c];
				}
			}
		}
		int bottomBorder = row - 1 - pady;
		for (int i = row - 1; i > bottomBorder; --i) {
			for (int j = 0; j < col; ++j) {
				for(int c = 0; c < srcChannel; c++) {
					newData[i * col * srcChannel + j * srcChannel + c] = newData[bottomBorder * col * srcChannel + j * srcChannel + c];
				}
			}
		}
	}

	memcpy(outImage, newData, col * row * srcChannel);
...
int saltNoise(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int num){
	...
	for (int i = 0; i < num / 2; i++)
	{
		...
		y = rand() % srcHeight;
		for(int c = 0; c < srcChannel; c ++){
			dstData[y * srcWidth * srcChannel + x * srcChannel + c] = 255;
		}
	}
...
int pepperNoise(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int num){
	...
	for (int i = 0; i < num / 2; i++)
	{
		...
		y = rand() % srcHeight;
		for(int c = 0; c < srcChannel; c ++){
			dstData[y * srcWidth * srcChannel + x * srcChannel + c] = 0;
		}
	}
...
int meanBlur(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int m, int n) {
    ...
    int col = srcWidth + padx * 2;
    unsigned char* newData = (unsigned char*)malloc(sizeof(unsigned char) * row * col * srcChannel);
	Padding(srcImage, srcWidth, srcHeight, srcChannel, newData, padx, pady, 1);
	...
	for (int i = 0; i < row - m + 1; ++i) {
		for (int j = 0; j < col - n + 1; ++j) {
			for (int c = 0; c < srcChannel; c++) {
				...
				for (int k = i; k < m + i; ++k) {
					for (int l = j; l < n + j; ++l) {
						sum += newData[k * col * srcChannel + l * srcChannel + c];
					}
				}
				int value = sum / nums;
...
int medianBlur(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int m, int n) {
	...
	int flag = 0;
	int nums = m * n;
	int* ker = (int*)malloc(m * n * sizeof(int));
	for (int i = 0; i < row - m + 1; ++i) {
		for (int j = 0; j < col - n + 1; ++j) {
			for (int c = 0; c < srcChannel; c++) {
				int idx = 0;
				for (int k = i; k < m + i; ++k) {
					for (int l = j; l < n + j; ++l) {
						ker[idx++] = newData[k * col * srcChannel + l * srcChannel + c];
					}
				}
...
int gaussianBlur(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int m,int n, float sigmaX,float sigmaY){
    ...
    if (sigmaX <= 0) sigmaX = 0.3 * ((m - 1) * 0.5 - 1) + 0.8;
    if (sigmaY <= 0) sigmaY = sigmaX;
	int pady =  m / 2;
	int padx =  n / 2;

	// 生成滤波核
    float* kernel = (float*)malloc(sizeof(float) * m * n);
    float sum = 0;
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
	        int x = i - pady;
	        int y = j - padx;
			float g = exp((x * x + y * y) * -0.5 / (sigmaX * sigmaY));
			sum += g;
        	kernel[i * n + j] = g;
        }
    }
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
    		kernel[i * n + j] = kernel[i * n + j] * 1.0 / sum;
        }
    }
    ...
    int col = srcWidth + padx * 2;
    unsigned char* newData = (unsigned char*)malloc(sizeof(unsigned char) * row * col * srcChannel);
	Padding(srcImage, srcWidth, srcHeight, srcChannel, newData, padx, pady, 0);
    ...
    for (int i = 0; i < row - m + 1; ++i) {
        for (int j = 0; j < col - n + 1; ++j) {
	        for(int c = 0; c < srcChannel; c++){
		        ...
		        for (int k = 0; k < m; ++k) {
			        for (int l = 0; l < n; ++l) {
			        	sum += kernel[k * n + l] * (float)newData[(k + i )* col * srcChannel + (l + j) * srcChannel + c];
			        }
		        }
		        if (sum > 255) {
		        	dstData[num] = 255;
...
int sobel(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int dx, int dy, int ksize){
	...
	// 边界填充
    unsigned char* newData = (unsigned char*)malloc(sizeof(unsigned char) * row * col * srcChannel);
	Padding(srcImage, srcWidth, srcHeight, srcChannel, newData, n / 2, m / 2, 1);
    ...
    for (int i = 0; i < row - m + 1; ++i) {
		for (int j = 0; j < col; ++j) {
	        for(int c = 0; c < srcChannel; c ++){
		        ...
		        for (int k = 0; k < n; ++k) {
			        temp[j * srcChannel + c] += ky[k] * newData[(k + i) * col * srcChannel + j * srcChannel + c];
		        }
				if (use_allDirect) {
					_temp[j * srcChannel + c] = 0;
					for (int k = 0; k < n; ++k) {
						_temp[j * srcChannel + c] += kx[k] * newData[(k + i) * col * srcChannel + j * srcChannel + c];
					}
				}
			}
        }
	...
	if (use_allDirect) {
		...
		for (int i = 0; i < row_; ++i) {
			for (int j = 0; j < col_; j++) {
				int val = dstData[i * col_ + j] + _dstData[i * col_ + j];
				if (val > 255) {
...
int laplacian(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int ksize){
	...
	if (ksize == 1 || ksize == 3) {
		...
		float** temp = (float**)malloc(sizeof(float*)*m);
		for (int i = 0; i < m; ++i) {
		...
		for (int i = 0; i < row - m + 1; ++i) {
			for (int j = 0; j < col - n + 1; ++j) {
				for(int c = 0; c < srcChannel;c++){
					...
					for (int k = 0; k < m; ++k) {
						for (int l = 0; l < n; ++l) {
							sum += temp[k][l] * newData[(k + i) * col * srcChannel + (l + j) * srcChannel + c];
						}
					}
					//边缘处理
					if (sum > 255) {
...
int canny(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int gaussianSize, int low_threshold, int high_threshold) {
	...
	if (srcChannel == 3){
		...
		gaussianBlur(dstData, gsWidth, gsHeight, gsChannel, gsImage, &gsWidth, &gsHeight, &gsChannel, 3,3, 0.8,0.8);
	} else {
		gaussianBlur(srcImage, srcWidth, srcHeight, srcChannel, gsImage, &gsWidth, &gsHeight, &gsChannel, 3,3, 0.8,0.8);
	}
	...
	sobel(gsImage, gsWidth, gsHeight, gsChannel, sobelyImage, &sobelyWidth, &sobelyHeight, &sobelyChannel, 0, 1, gaussianSize);
			...
			for(int c = 0; c < srcChannel; c++){
				...
				for (int k = i; k < ksize + i; ++k) {
					for (int l = j; l < ksize + j; ++l) {
						...
						double space = exp(-0.5 * (pow(k - M, 2) + pow(l - N, 2)) / sigma_space * sigma_space);
						double color = exp(-0.5 * pow(abs(newData[k][l * srcChannel + c] - pixel) / sigma_color, 2));
						sum += space * color * newData[k][l * srcChannel + c];
						normal += space * color;
					}
				}
				double tmp = sum / normal;
				if (tmp < 0) {
					dstData[flag] = 0;
				}
				else if (tmp > 255) {
					...
					dstData[flag] = static_cast<unsigned char>(tmp);
				}
				flag++;
			}
        }
...
int substract(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* subImage, int subWidth, int subHeight, int subChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int scale, bool use_abs){

	unsigned char* dstData = (unsigned char*)malloc(srcWidth * srcHeight * srcChannel);

	int widthC = srcWidth;
	int heightC = srcHeight;
	int channelC = srcChannel;
	int widthB = subWidth;
	int heightB = subHeight;
	int channelB = subChannel;
	int iOffsC, iOffsB, bValB, gValB, rValB, bValC, gValC, rValC;
	...
	for (int y = 0; y < heightC; y++)
	{
		...
		{
			iOffsC = (y * widthC + x) * channelC;
			iOffsB = (y * widthB + x) * channelB;
			//** 先取图B通道值
			if (x >= widthB || y >= heightB) // 不在图B范围内
			{
				// 不在图B范围内的点，直接输出图1
				if (channelC == 3)
				{
					dstData[iOffsC + 0] = srcImage[iOffsC + 0];
					dstData[iOffsC + 1] = srcImage[iOffsC + 1];
					dstData[iOffsC + 2] = srcImage[iOffsC + 2];
				}
				else
				{
					dstData[iOffsC + 0] = srcImage[iOffsC + 0];
			...
			else // 在图B范围内
			{
				...
				if (channelB == 3)
				{
					bValB = subImage[iOffsB + 0];
					gValB = subImage[iOffsB + 1];
					rValB = subImage[iOffsB + 2];
				}
				// 否则默认为单通道，视为同值3通道处理
				else
				{
					gValB = bValB = rValB = subImage[iOffsB + 0];
				}
				
				// 计算 A/B*255
				if (channelC == 3)
				{
					bValC = (srcImage[iOffsC + 0] - bValB) * scale;
					gValC = (srcImage[iOffsC + 1] - gValB) * scale;
					rValC = (srcImage[iOffsC + 2] - rValB) * scale;
					
					if (bValC < 0) bValC = use_abs ? -bValC : 0;
					if (gValC < 0) gValC = use_abs ? -gValC : 0;
					if (rValC < 0) rValC = use_abs ? -rValC : 0;

					dstData[iOffsC + 0] = bValC > 255 ? 255 : bValC;
					dstData[iOffsC + 1] = gValC > 255 ? 255 : gValC;
					dstData[iOffsC + 2] = rValC > 255 ? 255 : rValC;
				}
				else
				{
					bValC = (srcImage[iOffsC + 0] - bValB) * scale;
...
int equalize(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel) {
	...
	int val;
	float scale = 255.0 / (srcWidth * srcHeight);
	int hist[3][256];
	...
	for (int j = 0; j < srcChannel; j++) {
		...
		for (int i = 0; i < 256; i++) {
			histSum += hist[j][i];
			val = (int)(histSum * scale);
			if (val < 0)	lut[j][i] = 0;
			else if (val < 256)	lut[j][i] = val;
			else lut[j][i] = 255;
		}
	}
	...
	for (int i = 0; i < srcHeight; i++) {
		for (int j = 0; j < srcWidth; j++) {
			for (int c = 0; c < srcChannel; c++) {
				dstData[i * srcWidth * srcChannel + j * srcChannel + c] = lut[c][srcImage[i * srcWidth * srcChannel + j * srcChannel + c]];
			}
		}
	}
...
int meanPixel(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, int* meanPixel){
	...
	for (int i = 0; i < srcHeight; i++) {
		for (int j = 0; j < srcWidth; j++) {
			for (int c = 0; c < srcChannel; c++) {
				sum[c] += srcImage[i * srcWidth * srcChannel + j * srcChannel + c];
			}
		}
	}
	meanPixel[0] = sum[0] / size;
	meanPixel[1] = sum[1] / size;
	meanPixel[2] = sum[2] / size;
...
// threshold  7
int getDirectPixNum(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, bool isX, int pixel) {
	unsigned char* dstData = (unsigned char*)malloc(srcWidth * srcHeight);
	for (int i = 0; i < srcHeight; ++i) {
        for (int j = 0; j < srcWidth; j++) {
	...
	if (srcChannel == 1) {
		if (isX) {
			...
			for (int i = 0; i < srcHeight; ++i) {
				order = 0;
				for (int j = 0; j < srcWidth; j++) {
					if (srcImage[i * srcWidth + j] == pixel) {
						dstData[i * srcWidth + order] = 255;
						order++;
					}
				}
			}
			...
			for (int i = 0; i < srcWidth; ++i) {
				order = srcHeight - 1;
				for (int j = 0; j < srcHeight; j++) {
					if (srcImage[j * srcWidth + i] == pixel) {
						dstData[order * srcWidth + i] = 255;
						order--;
					}
				}
			}
...
int thresholdBinaryOTSU(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, float maxval, int region, float* thresh){
    ...
    for (int i = 0; i < srcHeight; ++i) {
	    for (int j = 0; j < srcWidth; j++)
	    {
			if (srcImage[i * srcWidth + j] > best_thresh) {
				dstData[i * srcWidth + j] = region == 1 ? (int)maxval : 0;
			}
			else {
				dstData[i * srcWidth + j] = region == 1 ? 0 : (int)maxval;
			}
	    }
    }
...
int thresholdBinaryAdaptive(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, float maxval, int region, int type, int ksize, int delta){

	unsigned char* thresData = (unsigned char*)malloc(srcWidth*srcHeight*srcChannel);
	if (type == 0) {
		meanBlur(srcImage, srcWidth, srcHeight, srcChannel, thresData, &srcWidth, &srcHeight, &srcChannel, ksize,ksize);
	}
	else if (type == 1) {
		gaussianBlur(srcImage, srcWidth, srcHeight, srcChannel, thresData, &srcWidth, &srcHeight, &srcChannel, ksize,ksize, 0,0);
	}
	else if (type == 2) {
		medianBlur(srcImage, srcWidth, srcHeight, srcChannel, thresData, &srcWidth, &srcHeight, &srcChannel, ksize,ksize);
	}
    ...
    for (int i = 0; i < srcHeight; ++i) {
	    for (int j = 0; j < srcWidth; j++) {
			if (srcImage[i * srcWidth + j] > thresData[i * srcWidth + j] - delta) {
				dstData[i * srcWidth + j] = region == 1 ? (int)maxval : 0;
			}
			else {
				dstData[i * srcWidth + j] = region == 1 ? 0 : (int)maxval;
...
int rotate(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, float clockwiseAngle, float* center){
	...
	for(int i = 0; i < srcHeight; ++i){
		for(int j = 0; j < srcWidth; ++j){
			int x = static_cast<int>(cosa * (j - center[0]) - sina * (-i + center[1]) + center[0] + 0.5);
			int y = static_cast<int>(-sina * (j - center[0]) - cosa * (-i + center[1]) + center[1] + 0.5);
			for(int c = 0; c < srcChannel; c++){
...
// feature
int connectedComponent(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int index, int conNums, int minConArea, int* area, int* center, int* rect) {
	...
	int conIdx = -1, growNum = 0, ptr = 0, curx = 0, cury = 0;
	int sumx = 0, sumy = 0;
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			if (srcImage[i * width + j] == 255 && flag[i * width + j] == 0 && conIdx < conNums - 1) {
				// 初始化本次连通域信息
				conIdx++;
				growNum = 1;
				ptr = 0;
				...
				if (conInfo[conIdx][0] < minConArea) {
					conIdx--;
				}
				else {
	...
	// 输出
	index -= 1;
	*area = conInfo[index][0];
	center[0] = conInfo[index][1];
	center[1] = conInfo[index][2];
	for (int i = 0; i < 8; i++) {
		rect[i] = conInfo[index][i + 3];
	}
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			outImage[i * width + j] = flag[i * width + j] == index + 1 ? 255 : 0;
		}
	}
	*outWidth = srcWidth;
	*outHeight = srcHeight;
...
int drawHoughLinesCV(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int* lines) {
	Mat src;
	if(srcChannel == 1){
		src = Mat::zeros(srcHeight, srcWidth, CV_8UC1);
	}else if(srcChannel == 3){
		src = Mat::zeros(srcHeight, srcWidth, CV_8UC3);
	}

	for (size_t i = 0; i < lines[0]; i++){
		line(src, Point(lines[i*4+1],lines[i*4+2]), Point(lines[i*4+3],lines[i*4+4]), Scalar(255,255,255), 1, 8);
	}
...
int warpPerspectiveCV(unsigned char* srcImage, int srcWidth, int srcHeight, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, float* srcPt, float *dstPt, int* size) {
	Mat srcImg, rstImg;
	if(srcChannel == 1){
		srcImg = Mat(srcHeight, srcWidth, CV_8UC1, srcImage);
		//rstImg = Mat(srcImg.size(), CV_8UC1);
	}else if(srcChannel == 3){
		srcImg = Mat(srcHeight, srcWidth, CV_8UC3, srcImage);
		//rstImg = Mat(srcImg.size(), CV_8UC3);
	}

	Point2f src[4] = {
		Point2f(srcPt[0], srcPt[1]),
		Point2f(srcPt[2], srcPt[3]),
		Point2f(srcPt[4], srcPt[5]),
		Point2f(srcPt[6], srcPt[7]),
	};
	Point2f dst[4] = {
		Point2f(dstPt[0], dstPt[1]),
		Point2f(dstPt[2], dstPt[3]),
		Point2f(dstPt[4], dstPt[5]),
		Point2f(dstPt[6], dstPt[7]),
	};
	Mat p = getPerspectiveTransform(src, dst);
	warpPerspective(srcImg, rstImg, p, Size(size[0], size[1]));

	memcpy(outImage, rstImg.data, rstImg.cols*rstImg.rows*srcChannel);
	*outChannel = srcChannel;
	*outHeight = rstImg.rows;
	*outWidth = rstImg.cols;
...
int findAllContoursCV(unsigned char* srcImage, int srcWidth, int srcLength, int srcChannel, unsigned char* outImage, int* outWidth, int* outHeight, int* outChannel, int selectSize, bool isRotated, float* centerPoint, float* rectPoint, int* conArea, int* conPointNum, int* conNum) {
	Mat img;
	if (srcChannel == 1) {
		Mat tmp = Mat(srcLength, srcWidth, CV_8UC1, srcImage);
		threshold(tmp, img, 127, 255, THRESH_BINARY | THRESH_OTSU);
	}
	else if (srcChannel == 3) {
	...
	Point2f vertices[4];
	if(selectSize > 0){
		*conNum = contours.size();
		*conPointNum = contours[contours.size() - selectSize].size();
		*conArea = (int)contourArea(contours[contours.size() - selectSize]);

		if (isRotated) { 
			RotatedRect rect = minAreaRect(contours[contours.size() - selectSize]);
			rect.points(vertices);
			centerPoint[0] = rect.center.x;
			centerPoint[1] = rect.center.y;
		}
		else {
			Rect rect = boundingRect(contours[contours.size() - selectSize]);
			centerPoint[0] = rect.x + rect.width / 2;
			centerPoint[1] = rect.y + rect.height / 2;
			vertices[0] = rect.tl();
			vertices[2] = rect.br();
			vertices[1] = Point2f(rect.x + rect.width, rect.y);
			vertices[3] = Point2f(rect.x, rect.y + rect.height);
		}
		rectPoint[0] = vertices[0].x;
		rectPoint[1] = vertices[0].y;
		rectPoint[2] = vertices[1].x;
		rectPoint[3] = vertices[1].y;
		rectPoint[4] = vertices[2].x;
		rectPoint[5] = vertices[2].y;
		rectPoint[6] = vertices[3].x;
		rectPoint[7] = vertices[3].y;
	}
	Mat imgCon = Mat::zeros(img.size(), CV_8UC1);
	for (int i = 0; i < hierarchy.size(); i++) {
		drawContours(imgCon, contours, i, Scalar(255,255,255), 0, 8, hierarchy);
		if(contours.size() - selectSize == i){
			//circle(imgCon, Point(centerPoint[1], centerPoint[2]), 2, Scalar(255,255,255));
